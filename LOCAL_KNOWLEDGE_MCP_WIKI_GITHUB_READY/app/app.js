(function () {
  ...
  const byType = (type) => pages.filter((p) => p.type === type);

  function navItem(page, active) {
    return `<div class="nav-item ${active ? "active" : ""}">${page.title}</div>`;
  }

  document.getElementById("riskList").innerHTML = byType("risk").map((p, i) => navItem(p, i === 0)).join("");
  document.getElementById("controlList").innerHTML = byType("control").map((p) => navItem(p, false)).join("");
  document.getElementById("frameworkList").innerHTML = byType("framework").map((p) => navItem(p, false)).join("");
  ...
  const selectedText = data.selected.page.text;
  const summaryMatch = selectedText.match(/## Summary\s+([\s\S]*?)(?=\n## )/);
  document.getElementById("summaryText").textContent = summaryMatch ? summaryMatch[1].replace(/\s+/g, " ").trim() : "";

  document.getElementById("controlCards").innerHTML = data.mapping.controls.map((control) => {
    return `<div class="card"><strong>${control.title}</strong><span>${(control.frameworks || []).join(" / ")}</span></div>`;
  }).join("");

  document.getElementById("mappingText").innerHTML = data.mapping.controls.map((c) => c.title).join("<br>");
  ...
  graph.innerHTML = edges.map(([a, b]) => edge(nodes[a], nodes[b])).join("") +
    nodes.map((n) => `<div class="node ${n.type}" style="left:${n.x}px;top:${n.y}px">${n.label}</div>`).join("");
})();
