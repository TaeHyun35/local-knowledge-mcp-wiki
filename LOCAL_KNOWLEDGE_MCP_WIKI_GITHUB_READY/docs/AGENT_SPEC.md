...
| Source Coverage | selected page의 declared/resolved source, Evidence Map, quality score | 출처 무결성 확인 |
| Evidence Trace | Raw Source → Wiki Page → Evidence Map → Quality Gate 흐름 | 주장 추적성 확인 |
| Reading Deck | 사용자가 검토 중인 page를 browser localStorage에 저장 | 반복 검토 workflow 확인 |
| 관계 그래프 | risk-control-framework nodes/edges, 고정 anchor 기반 edges | Wiki 관계 구조 확인 |
| 오른쪽 Agent 패널 | MCP Tool 목록, JSON-RPC 요청 예시, 선택 페이지 기준 결과 | Tools and Agent 확인 |
...
정적 GUI의 세부 패널은 위 Tool과 직접 연결된다. Source Coverage 카드는 `source_coverage`, Evidence Trace 단계는 `source_coverage`, `read_page`, `trace_claim`, `quality_report`, Special Wiki Tools 카드는 `quality_report`, `trace_claim`, `create_wiki_draft`를 선택하도록 구성되어 있다.
...
| `app/index.html` | Source Coverage, Evidence Trace, Reading Deck, fixed-anchor Wiki Graph 표시 |
